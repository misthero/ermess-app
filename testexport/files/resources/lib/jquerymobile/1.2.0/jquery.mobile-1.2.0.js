...
(function( $, window, undefined ) {
	...
	$.mobile = $.extend( {}, {
		...
		silentScroll: function( ypos ) {
			if ( $.type( ypos ) !== "number" ) {
				ypos = $.mobile.defaultindexScroll;
			}
...
// 
// Project index - http://benalman.com/projects/jquery-hashchange-plugin/
...
var createHandler = function( sequential ) {
	...
	return function( name, reverse, $to, $from ) {
		...
		var deferred = new $.Deferred(),
			...
			toScroll = active.lastScroll || $.mobile.defaultindexScroll,
			screenHeight = $.mobile.getScreenHeight(),
...
(function( $, undefined ) {
	...
	var $window = $( window ),
		...
		path = {
			...
			// This is usually to allow the application to "phone index" and fetch app specific
	...
	setLastScroll = function() {
		...
		if ( active ) {
			...
			active.lastScroll = lastScroll < $.mobile.minScrollBack ? $.mobile.defaultindexScroll : lastScroll;
		}
	};
...
(function( $, window ) {
	...
	$.extend( pushStateHandler, {
		...
		init: function() {
			...
			// if there's no hash, we need to replacestate for returning to index
			if ( location.hash === "" ) {
...
(function( $, window, undefined ) {
	...
	$(function() {
		...
		// if defaultindexScroll hasn't been set yet, see if scrollTop is 1
		// it should be 1 in most browsers, but android treats 1 as 0 (for hiding addr bar)
		// so if it's 1, use 0 from now on
		$.mobile.defaultindexScroll = ( !$.support.scrollTop || $( window ).scrollTop() === 1 ) ? 0 : 1;
